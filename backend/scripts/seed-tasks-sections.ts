...
import { orgMember, section, sectionMember, task, taskList, user } from '../src/db/schema.ts'
...
const sectionTaskMap: Record<string, TaskListSeed[]> = {
  'Kitchen Operations': [
    {
      name: 'Opening',
      tasks: [
...
const run = async () => {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`delete from task_lists`)
    ...
    for (const seededSection of insertedSections) {
      const listSeeds = sectionTaskMap[seededSection.name] || []
      if (listSeeds.length === 0) continue

      const insertedLists = await tx
        .insert(taskList)
        .values(
          listSeeds.map((listSeed, index) => ({
            sectionId: seededSection.id,
            name: listSeed.name,
            description: listSeed.description ?? null,
            sortOrder: index,
          }))
        )
        .returning({ id: taskList.id, name: taskList.name })

      for (const insertedList of insertedLists) {
        const listSeed = listSeeds.find((item) => item.name === insertedList.name)
        const taskSeeds = listSeed?.tasks ?? []
